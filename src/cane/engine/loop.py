...
`on_bar` เป็นของที่ฉีดเข้ามา ไม่มีค่าตั้งต้น — ตัวที่ต่อจริงของ live คือ `engine/live.py:LiveRunner` ซึ่ง `cane engine run`
ประกอบให้ (ใบ 13) ส่วน replay ใช้ `engine/replay.py` · ทั้งสองเรียก `run_bar()` ตัวเดียวกัน
