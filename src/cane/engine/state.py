...
#: คำที่คอนโซลแสดง · เป็นสตริงไม่ใช่ `Enum` เพราะมันไหลออกทาง JSON ของ
#: `/api/engine/status` ตรงๆ และ `Enum` จะบังคับให้มีขั้นแปลงที่ขอบทุกครั้ง
#:
#: `stopping` กับ `blocked` เป็นสองคำที่คนอ่านมักคิดว่าเกินมา — ไม่เกิน:
#: `stopping` คือช่วงที่สั่งหยุดแล้วแต่ยังไม่จบรอบ ซึ่งเป็นช่วงที่ยังส่งออเดอร์อยู่ได้
#: และ `blocked` คือ engine ที่ยังเดินแต่ไม่เทรดเพราะ config ใช้ไม่ได้ ซึ่งต่างจาก
#: `stopped` ตรงที่ไม่มีใครต้องไปกดสตาร์ท มีแต่ต้องไปแก้ config
STOPPED = "stopped"
RUNNING = "running"
CRASHED = "crashed"
STOPPING = "stopping"
