"""อ่าน/เขียน `verdict_cache` — กลไกที่ทำให้ "ตัดสินครั้งเดียวจบ" เป็นจริง (spec/04:62-76)
...
## `side` อยู่ในคีย์ทั้งที่ซ้ำซ้อนกับ `factor` — และเหตุผลไม่ใช่ที่สเปกเขียนไว้

spec/04:72 บอกว่า `side` ต้องอยู่ในคีย์ "เพราะแท่งเดียวกันเป็นได้ทั้งจุดปิด long และ
จุดเปิด short ถ้าไม่มี side คำตัดสินของสองฝั่งจะทับกันเงียบๆ" · **เหตุผลนั้นไม่จริง
ในรูปปัจจุบัน** — ชื่อ factor ของสองฝั่งไม่ซ้ำกันเลยสักตัว (`SIDE_OF_FACTOR` คือ
ฟังก์ชันสมบูรณ์จาก factor ไป side) คำถามของฝั่ง long จึงชนแถวของฝั่ง short ไม่ได้อยู่
แล้วแม้จะถอด `side` ออกจาก `WHERE`
...
class CacheKey:
    """คีย์เจ็ดช่องของ `verdict_cache` — หกช่องของสเปก บวก `market`

    `market` ไม่ได้อยู่ในคีย์ที่ spec/04:70 เขียนไว้ เพราะสเปกข้อนั้นเขียนก่อน ADR 26
    ที่ทำให้ `market` เป็นมิติของ symbol · ที่มาเต็มอยู่ในหัวไฟล์ของ migration 0007
