"""อ่าน/เขียน `verdict_cache` — กลไกที่ทำให้ "ตัดสินครั้งเดียวจบ" เป็นจริง (spec/04:69-77)
...
spec/04:75 บอกว่า `side` อยู่ในคีย์เพราะมันอยู่ใน PK คู่กับ CHECK ที่ผูก factor↔side
...
class CacheKey:
    """คีย์เจ็ดช่องของ `verdict_cache` ตาม spec/04:74
