"""ถาม LLM ทีละ factor ผ่าน cache แล้วคืนคำตัดสินของทั้งฝั่ง (spec/04, spec/08 §สิบสี่ขั้นของหนึ่งรอบ)
...
spec/04 §ข้อบังคับเรื่องความคงเส้นคงวา บอกให้ตั้งค่าการสุ่มต่ำสุด**เท่าที่ API นั้นรองรับ** ซึ่งเป็นเรื่องของ
...
**`model_id` อยู่ใน hash ด้วย** (spec/04 §ข้อบังคับเรื่องความคงเส้นคงวา) การเปลี่ยนโมเดลมีผลต่อคำตอบไม่น้อยกว่า
การแก้ถ้อยคำ ถ้าไม่รวม การสลับโมเดลจะอ่านคำตัดสินของโมเดลเก่ามาใช้ต่อเงียบๆ ซึ่งเป็น
การ "หลอก" ข้อเดียวกับที่สเปกตั้งใจกัน (spec/04 §ข้อบังคับเรื่องความคงเส้นคงวา) · ค่าที่ adapter ส่งมาเป็น
...
#: JSON schema ที่บังคับรูปคำตอบ (spec/04 §สัญญาผลลัพธ์ (contract)) · `additionalProperties: false` กับ
...
class JudgeResult:
    """ผลของการตัดสินทั้งฝั่ง — ตรงกับช่องที่ `DecisionRecord` ต้องกรอก (spec/07 §บันทึกการตัดสินใจ)

    `fallback` แยก "LLM ตัดสินว่าไม่มีปัจจัย" ออกจาก "LLM ตอบไม่ได้" ซึ่ง spec/04 §เมื่อ LLM ใช้การไม่ได้
...
def prompt_text(factor: str) -> str:
    ...
    กับ `prompts/short/` เป็นคนละชุดตาม spec/04 §prompt แยกสองชุด ไม่ใช่ template ที่สลับคำ
    """
...
def judge_side(
    ...
    *,
    ...
    """ตัดสิน 3 factor ของฝั่งที่กำลังจะเข้า · ไม่ถามฝั่งตรงข้ามเลย (spec/04 §หก factor — สามต่อฝั่ง)

    ถามทีละตัวตามลำดับ ไม่ขนาน — spec/08 §สิบสี่ขั้นของหนึ่งรอบ บอกว่า "ขนานกันได้" ซึ่งเป็นการอนุญาต
