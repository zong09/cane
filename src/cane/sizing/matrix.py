...
— spec/05 §ขนาดไม้ของเส้นทาง cold start บอกว่าต้องแยกออกว่าไม้เล็กเพราะปัจจัยน้อยหรือเพราะชนเพดาน และการเก็บแค่
...
(spec/05 §leverage อยู่ตรงไหนของสูตร) · ผลคือ `leverage = 1` ของ spot ทำให้ `notional == margin` เองโดยไม่ต้อง
...
#: ปัจจัยละกี่เปอร์เซ็นต์ (spec/05 §สูตร) · เอกสารต้นทางเขียน "+20" ต่อปัจจัย
...
#: เพดานของสูตรก่อนเพดานของ risk (spec/05 §สูตร) · ที่ `base_pct` สูงสุด 20 สูตรให้ 80
...
def size_pct(
    *, base_pct: float, factors_present: int, max_position_pct: float
    ...
    เรียก Judge (spec/05 §ขนาดไม้ของเส้นทาง cold start) — มันจึงเรียกที่นี่ด้วย `factors_present = 0` ได้ตรงๆ
...
def floor_to_step(value: float, step: float) -> float:
    """ปัดลงให้เป็นจำนวนเท่าของ `step` · **ปัดลงเสมอ ไม่มีกรณีปัดขึ้น** (spec/05 §สูตร)
...
def plan_size(
    *,
    ...
    """สูตรเต็มของ spec/05 §สูตร · ทุกค่าต้องมาจาก**ฝั่งเดียวกัน**หมด
