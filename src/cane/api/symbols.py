...
def remove(
    ...
    **รหัสรับทาง body ทางเดียว** — ค่าตั้งต้นของ htmx ส่งค่าของ `DELETE` ไปใน query
    string ซึ่งลงไปนอนอยู่ใน access log ของทุกชั้นที่คำขอผ่าน · ทางแก้อยู่ที่ต้นทาง
    (`methodsThatUseUrlParams` ใน `base.html` เหลือแค่ `get`) ไม่ใช่ที่นี่: ปลายทางที่
    ยอมรับรหัสจาก URL ด้วยแปลว่าใครก็ยังส่งมาทางนั้นได้ แล้วความลับก็ไปโผล่ใน log อยู่ดี
