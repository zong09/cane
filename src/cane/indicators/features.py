"""feature ที่ป้อน Confluence Judge — ตัวเลขล้วน ไม่มีคำตัดสิน (spec/04 §LLM เห็นอะไร)
...
spec/04 §ทำไมต้องใช้ LLM ระบุไว้ตรงๆ ว่าคำว่า "สำเร็จ" "ขนาดใหญ่" "อย่างชัดเจน" ไม่มีเกณฑ์ตายตัว
...
`evidence_bars` ของ `ConfluenceVerdict` (spec/04 §สัญญาผลลัพธ์ (contract)) เป็น `[int]` ที่ LLM ต้องอ้างถึงได้
...
1. **"ขนาด body เทียบ ATR (z-score)"** (spec/04 §LLM เห็นอะไร) อ่านได้ทั้ง "body ÷ ATR" และ
...
2. **"linear fit"** (spec/04 §LLM เห็นอะไร) ฟิตบนอะไร · ที่นี่ฟิตบน **จุด pivot** ไม่ใช่ high/low
...
#: คาบ ATR แบบ Wilder · **ไม่ใช่ 5/10 ของ CDC ATR Trailing Stop** (spec/03 §ทางที่ 2 — `trailing` CDC ATR Trailing Stop)
...
@dataclass(frozen=True, slots=True)
class Features:
    """feature ของแท่งสุดท้ายใน `bars` — หนึ่งฟิลด์ต่อหนึ่งบรรทัดในตารางของ spec/04 §LLM เห็นอะไร
...
def wilder_atr(bars: Sequence[Bar], length: int = ATR_PERIOD) -> float:
    ...
    **ยืนยันกับไฟล์จริงจาก TradingView แล้ว** (`tests/test_trailing.py`) — ต่างจาก
    การ seed EMA ที่คู่มือ Pine เขียนไว้ไม่ตรงกับของจริง ข้อนี้ตรง · เส้น `Fast
    Trail` / `Slow Trail` ในไฟล์ export เว้นค่าไว้ `period - 1` แท่งพอดี ซึ่งแปลว่า
    ATR ตัวแรกครอบ TR ของแท่ง 0..`period-1` = `period` ตัวโดยนับแท่งแรกเข้าด้วย
    จริง · และเส้น trail ตรงกับเราทั้ง 2564 แท่งโดยไม่มีความคลาดเคลื่อนเลย ซึ่งเป็น
    หลักฐานที่แรงกว่าการเทียบ ATR ตรงๆ เพราะ `Trail` พาผลของ seed ไปทั้งเส้นผ่าน
    `max`/`min` แทนที่จะให้มันจางไปตามเวลาแบบ EMA
...
def pivot_highs(
    ...
    """ยอดที่ยืนยันแล้ว — ภาพสะท้อนของ `pivot_lows()` ทีละข้อ (spec/04 §หก factor — สามต่อฝั่ง)"""
