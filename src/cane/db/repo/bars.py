...
แทน `data/cache.py` (JSON ไฟล์ต่อ symbol/timeframe) ที่ถูกลบไปในใบ 02b
"""
...
def insert_bars(
    ...
    **`ON CONFLICT DO NOTHING` ไม่ใช่ upsert** สองเหตุผลที่ต้องเป็นทางนี้:
    ...
    เวลานาฬิกาของผู้เรียก จึงกรองซ้ำให้ไม่ได้ — `_load()` ของ `data/ohlcv.py` เป็น
    ด่านนั้น และเป็นด่านเดียว
