...
แทน `DecisionRecord` แบบ JSONL ที่ spec/07 §บันทึกการตัดสินใจ เขียนไว้ (decisions #22 ย้ายลงฐาน)
หัวใจไม่เปลี่ยน: **เขียนทุกกรณี รวมถึงตอนที่ผลคือ "ไม่ทำอะไร"** (decisions #11, spec/08 §กฎที่ห้ามผิดลำดับ)
...
@dataclass(frozen=True, slots=True)
class RiskCheck:
    """ผลการตรวจ risk หนึ่งชั้น · `seq` คือลำดับที่ชั้นนั้นถูกเรียกจริง (spec/08 §สิบสี่ขั้นของหนึ่งรอบ)"""
...
@dataclass(frozen=True, slots=True)
class Stop:
    """stop order ของแท่งนี้ · `action = missing` คือหา stop ที่ควรมีไม่เจอ (spec/08 §การเลื่อน stop ตาม Slow Trail)"""
...
class DecisionRecord:
    ...
    ด้วย `no_signal` มีแค่หัวกับ `zone`/`state`/`close_px` (spec/08 §กฎที่ห้ามผิดลำดับ)
...
def _check_skip_reason(record: DecisionRecord) -> None:
    ...
    if not accepted and record.skip_reason is None:
        raise ValueError(
            ...
            "แท่งที่ไม่ได้เข้าไม้ต้องบอกได้ว่าเพราะอะไร (spec/08 §กฎที่ห้ามผิดลำดับ)"
        )
...
def _check_risk_sequence(record: DecisionRecord) -> None:
    ...
    spec/08 §สิบสี่ขั้นของหนึ่งรอบ ตรวจทีละชั้นและชั้นแรกที่ไม่ผ่านปฏิเสธทั้งไม้ → ชั้นที่ตามหลังชั้นที่
