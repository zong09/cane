...
# (decisions #11, spec/08 §กฎที่ห้ามผิดลำดับ) — บันทึกที่เขียนเฉพาะตอนมีออเดอร์คือบันทึกที่เข้าข้างตัวเอง
...
#: เขียนไม่ลง ซึ่งกลับหัวเจตนาทั้งใบ (spec/08 §กฎที่ห้ามผิดลำดับ)
decisions = Table(
    "decisions",
    metadata,
    Column("id", BigInteger, Identity(always=True), primary_key=True),
...
#:
#: `missing` คือการทำตาม spec/08 §การเลื่อน stop ตาม Slow Trail ที่ห้ามวาง stop ใหม่เงียบๆ ทับตอนหา stop เดิมไม่เจอ
...
#: สวิตช์หยุดฉุกเฉิน — **สภาพปัจจุบัน ไม่ใช่ประวัติ** (spec/10 §5. state ที่อยู่ในตาราง)
#:
