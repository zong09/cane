...
การ seed ค่าเริ่มต้นของ EMA ต่างกันได้ระหว่าง Pine กับ `pandas.ewm(adjust=False)` และนั่นคือจุดที่แท่งช่วงต้นจะไม่ตรงกันมากที่สุด **ต้องยืนยันด้วยข้อมูลจริง ไม่ใช่ด้วยการอ่านโค้ด** — ยืนยันแล้วด้วยไฟล์ export จริง (2564 แท่ง 1D): `ta.ema` คืน `na` ตลอด `length - 1` แท่งแรกแล้ว seed ด้วย SMA ของ `length` แท่งแรก **ไม่ใช่** ค่าแรกของ source ตามที่คู่มือ Pine เขียนไว้ · ถ้ายังไม่ตรง ให้แก้ที่วิธี seed จนตรง ไม่ใช่ขยายช่วงที่ตัดทิ้ง
