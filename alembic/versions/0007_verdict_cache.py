"""verdict_cache — คำตัดสินหนึ่งครั้งต่อแท่ง เรียกซ้ำได้คำตอบเดิมเสมอ (spec/04:62-76)
...
## ข้อ 1 ของสเปก (`temperature 0`) ทำไม่ได้แล้ว — ข้อ 2 (ตารางนี้) จึงต้องแบกทั้งหมด

spec/04:64 สั่ง `temperature 0` คู่กับ structured output · โมเดล Claude รุ่นปัจจุบัน
(Opus 5, Sonnet 5, ตระกูล 4.6 ขึ้นไป) **ถอด `temperature` ออกจาก API แล้ว ส่งไปได้ 400**
ไม่ใช่ตั้งไม่ได้ คือส่งไม่ได้เลย · เรื่องนี้ไม่ได้ทำให้เจตนาของสเปกพัง เพราะ temperature 0
ไม่เคยรับประกัน determinism ของ LLM อยู่แล้ว — ตัวที่รับประกันจริงคือ "ตัดสินครั้งเดียว
แล้วอ่านของเดิม" ซึ่งคือตารางนี้ **สเปกข้อ 1 ควรถูกแก้ ผู้เขียน migration ไม่แก้สเปกให้**

## คีย์มี `market` ซึ่งสเปกไม่ได้เขียนไว้ — และมันจำเป็น

spec/04:70 เขียนคีย์เป็น `(symbol, timeframe, bar_close_ts, side, factor, prompt_hash)`
ซึ่งเขียนไว้**ก่อน** ADR 26 ทำให้ `market` กลายเป็นมิติของ symbol · ตอนนี้ `BTC/USDT` บน
`spot` กับบน `usdtm_perp` เป็นคนละแท่งจริงๆ (ตาราง `bars` มี `market` ใน PK) คำตัดสินที่
คำนวณจากแท่งคนละชุดจึงทับกันไม่ได้ · ถ้าไม่มี `market` ในคีย์ ไม้ spot จะได้คำตัดสินของ
perp มาใช้เงียบๆ ซึ่งเป็นการหลอกแบบเดียวกับที่สเปกตั้งใจกัน
