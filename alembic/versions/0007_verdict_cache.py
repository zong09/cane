"""verdict_cache — คำตัดสินหนึ่งครั้งต่อแท่ง เรียกซ้ำได้คำตอบเดิมเสมอ (spec/04 §ข้อบังคับเรื่องความคงเส้นคงวา)
...
spec/04 §ข้อบังคับเรื่องความคงเส้นคงวา สั่งให้ตั้งค่าการสุ่มต่ำสุดเท่าที่ API รองรับ **ซึ่งไม่ใช่หลักประกัน** ·
`temperature 0` ตั้งได้ แต่ endpoint ที่ทำ batching รวมผลบวกทศนิยมคนละลำดับตามองค์
ประกอบของ batch พอ logit ขยับ greedy ก็เลือกคนละ token · ตัวที่รับประกันจริงคือ
"ตัดสินครั้งเดียวแล้วอ่านของเดิม" ซึ่งคือตารางนี้

*(ตอนเขียน migration นี้ สเปกข้อ 1 ยังสั่ง `temperature 0` ตรงๆ · แก้แล้ว)*

## คีย์มี `market` — spec/04 §ข้อบังคับเรื่องความคงเส้นคงวา เขียนตรงกันแล้ว
...
def upgrade() -> None:
    op.create_table(
        ...
        # (spec/04 §สัญญาผลลัพธ์ (contract)) · บังคับที่ฐานด้วย ไม่ใช่เชื่อว่า `validate()` ถูกเรียกเสมอ
        sa.CheckConstraint(
            "present = false"
